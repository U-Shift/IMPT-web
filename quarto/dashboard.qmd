...
DATABASE = [
  {
    ...
    source: "/IMPT-web/www/data/sample/0800.geojson",
  ...
  },
  {
    ...
    source: "/IMPT-web/www/data/sample/2300.geojson",
    map_style: (feature) => GRADUATE_POLYGON(feature, 270),
    map_onEachFeature: (feature, layer) => FEATURE_TOOLTIP(feature, layer, "Freguesia", [
        {key: "Concelho", label: "Municipality"},
        {key: "services", label: "Services", format: (v) => parseInt(v)},
  ...
  },
  {
    ...
    source: "/IMPT-web/www/data/sample/metro_stations.geojson",
  ...
  },
  {
    ...
    source: "/IMPT-web/www/data/sample/sport_centers.geojson",
  ...
  {
    label: "Cycle lanes",
    source: "/IMPT-web/www/data/sample/cycle_lanes.geojson",
    map_onEachFeature: (feature, layer) => FEATURE_TOOLTIP(feature, layer, "DESIGNACAO", [
...
viewof STATE_ZCOL = Inputs.radio(
  STATE_DATA.zcol instanceof Array ? STATE_DATA.zcol : [STATE_DATA.zcol],
  {
    ...
    disabled: STATE_DATA.zcol.length<=1
  }
);

viewof STATE_LIGHTMODE = Inputs.button("🌓 Toggle base map", {value: true, reduce: (light) => {
   L.tileLayer(light ? MAP_DARK : MAP_LIGHT).addTo(STATE_MAP);
   return !light;
}});

viewof DOWNLOAD = Inputs.button("📦 Download layer", {value: true, reduce: () => {
...
${STATE_ZCOL && STATE_DATA.cols && STATE_DATA.cols[STATE_ZCOL] ?  STATE_DATA.cols[STATE_ZCOL] : STATE_ZCOL} frequency plot

```{ojs, Data plot}
STATE_ZCOL ? Plot.auto(DATASET.features.map(f => f.properties), {x: STATE_ZCOL}).plot() : html`Please, select a lot indicator ;)`
```
